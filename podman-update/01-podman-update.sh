...
if [ ! -f $PODMAN_DL ]; then
  curl -fsSLo $PODMAN_DL https://github.com/OpenHorse/udm-utilities/blob/master/podman-update/bin/podman-$PODMAN_VERSION?raw=true
fi
chmod +x $PODMAN_DL
...
if [ ! -f $RUNC_DL ]; then
  curl -fsSLo $RUNC_DL https://github.com/OpenHorse/udm-utilities/blob/master/podman-update/bin/runc-$RUNC_VERSION?raw=true
fi
chmod +x $RUNC_DL
...
if [ ! -f $SECCOMP ]; then
  ...
  curl -fsSLo /usr/share/container/seccomp.json https://github.com/OpenHorse/udm-utilities/blob/master/podman-update/bin/seccomp.json?raw=true
fi
